 ...
 * 
 * This software is a proof of concept implementation for an
 * automated and attack graph-based evaluation of on-board networks.
 * First, please run all JUnit tests for ensuring a working installation,
 * e.g. analysis.AnalyzerTest.java.
 * Then use this main file or a new JUnit test method for executing analyses.
 * 
...
public class Main {
	...
	/** Main method.
	 * @param args are not necessary. Will be ignored.
	 */
...
//		maxMemory();
	...
	/**
	 * Show that any massive path occurrence within an network will lead
	 * to far too high probabilities.
	 */
	...
	private static final void expectedValueTest() {
		...
		/* 64-bit run */
		for (int n=1 ; n<=500 ; n++) {
//			System.out.println("n=" + n + " => Exp:" + expectedValue(pathHitLikelihood, pathCost, n));
	 ...
	 * @param pathNumber
	 */
	private static double expectedValue(final double pathHitLikelihood,
			final int pathCost, final int pathNumber) {
		double expectedValue = 0;
//		expectedValue += pathCost * pathHitLikelihood; // first try a hit case.
		for (int n=1 ; n<=pathNumber ; n++) { // every path that could trigger a success.
	 ...
	 * @param pathNumber
	 */
	private static float expectedValue32bit(final float pathHitLikelihood,
			final int pathCost, final int pathNumber) {
