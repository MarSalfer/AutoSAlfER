...
public class CostSoFarComparatorTest {
	...
	public void setUp() throws Exception {
		...
		sysModel.addNode(s2);
		sysModel.addNode(s3);
		Access ac = new Access("XSS-Injection", s1, 1_000_000);
		...
		c0 = new CrawledAccess(scenario, ac);
	...
	/**
	 * Test method for {@link analysis.comparator.CostSoFarComparator#compare(analysis.Crawlable, analysis.Crawlable)}.
	 */
	@Test
	public final void testAttackVectors() {
		c1.addNewExploitOrigin(c0, exploit1, 1d); // Necessary for the CostSoFarComparator.
		c2.addNewExploitOrigin(c1, exploit3, 2d); // Necessary for the CostSoFarComparator.
		c3.addNewExploitOrigin(c1, exploit6, 3d); // Necessary for the CostSoFarComparator.
			
		// Sorting is to be determined by costs so far.
		TreeSet<Crawlable> ts = new TreeSet<Crawlable>(new CostSoFarComparator());
		ts.add(c1);
		ts.add(c2);
		ts.add(c3);
		...
		System.out.print(c3);
		System.out.println(c3.getCostForPathSoFar());
		System.out.println();
		...
		*/
		...
		assertEquals(c1, ts.first());
		assertEquals(c1, ts.first());
		assertEquals(c1, ts.first());
		assertEquals(c3, ts.last());
		assertEquals(c3, ts.last());
		assertEquals(c3, ts.last());
		assertEquals(c1, ts.pollFirst());
		assertEquals(c2, ts.pollFirst());
		assertEquals(c3, ts.pollFirst());
		assertEquals(null, ts.pollFirst());
	}
}
